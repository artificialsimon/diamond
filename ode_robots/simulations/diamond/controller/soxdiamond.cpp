...
SoxDiamond::SoxDiamond(const SoxDiamondConf& conf)
  : AbstractController("SoxDiamond", "1.1"),
    conf(conf)
{
  constructor();
}
...
SoxDiamond::SoxDiamond(double init_feedback_strength, bool useExtendedModel, bool useTeaching )
  : AbstractController("SoxDiamond", "1.1"),
    conf(getDefaultConf()){
...
SoxDiamond::~SoxDiamond(){
}


void SoxDiamond::init(int sensornumber, int motornumber, RandGen* randGen){
...
sensor* SoxDiamond::getPredictionState() {
  /** Call it after step() */
...
matrix::Matrix SoxDiamond::getA(){
  return A;
}

matrix::Matrix* SoxDiamond::getpA(){
  return &A;
}

void SoxDiamond::setA(const matrix::Matrix& _A){
  assert(A.getM() == _A.getM() && A.getN() == _A.getN());
  A=_A;
}

matrix::Matrix SoxDiamond::getC(){
  return C;
}

void SoxDiamond::setC(const matrix::Matrix& _C){
  assert(C.getM() == _C.getM() && C.getN() == _C.getN());
  C=_C;
}

matrix::Matrix SoxDiamond::geth(){
  return h;
}
...
void SoxDiamond::step(const sensor* x_, int number_sensors,
...
Matrix SoxDiamond::pseudoInvL(const Matrix& L, const Matrix& A, const Matrix& C){
  if(pseudo == 0){
...
void SoxDiamond::setMotorTeaching(const matrix::Matrix& teaching){
...
void SoxDiamond::setSensorTeaching(const matrix::Matrix& teaching){
...
matrix::Matrix SoxDiamond::getLastMotorValues(){
  return y_buffer[(t-1+buffersize)%buffersize];
}

matrix::Matrix SoxDiamond::getLastSensorValues(){
  return x_buffer[(t-1+buffersize)%buffersize];
}

list<Matrix> SoxDiamond::getParameters() const {
  return {C,h};
}

int SoxDiamond::setParameters(const list<Matrix>& params){
  if(params.size() == 2){
...
bool SoxDiamond::store(FILE* f) const{
  // save matrix values
  C.store(f);
  h.store(f);
  A.store(f);
...
bool SoxDiamond::restore(FILE* f){
  // save matrix values
  C.restore(f);
  h.restore(f);
  A.restore(f);
